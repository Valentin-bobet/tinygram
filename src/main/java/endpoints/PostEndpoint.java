...
//		datastore.put(pi);
	...
	public Entity Delete(User user, Post post) throws UnauthorizedException {
		...
		Transaction deletePostTransaction = datastore.beginTransaction();
		Key postKey = KeyFactory.createKey("Post", post.getId());

		datastore.delete(postKey);
		deletePostTransaction.commit();
